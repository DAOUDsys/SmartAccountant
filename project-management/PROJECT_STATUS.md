...
Latest atomic accounting success audit update: posting, reversal, and DRAFT cancellation now write mandatory SUCCESS audit events through `AuditLogService.createEvent` inside the same Prisma transaction as the accounting/domain mutation. Posting writes `TRANSACTION_POST_SUCCEEDED` after the posted journal, balanced journal lines, and transaction status update are prepared; same-key idempotent retries return the existing journal without creating a duplicate success event. Reversal writes `TRANSACTION_REVERSAL_SUCCEEDED` inside the existing Serializable reversal transaction after creating the reversal journal, marking the original journal `REVERSED`, and voiding the source transaction. DRAFT cancellation writes `TRANSACTION_DRAFT_VOIDED` in the same transaction that sets `Transaction.status = VOIDED` and void metadata. Trusted audit context is derived in controllers from the authenticated user and request/correlation middleware, with API/MOBILE transport mapped safely and `MANUAL`, `SYSTEM_RETRY`, and `IMPORT` operation sources mapped without letting clients choose arbitrary `AuditSource` values. Audit metadata stores allowlisted fields only, including SHA-256 idempotency-key fingerprints instead of raw keys. Validation passed with `npm run lint`, `npm run build`, `npm run test` (26 files, 244 tests), `npm run prisma:validate`, and `npm run prisma:generate`. Docker Desktop/PostgreSQL is currently unavailable on this workstation, so final live smoke, direct database inspection, and clean migration-status verification are pending; `docker compose up -d postgres` failed because the Docker Desktop Linux engine pipe was not available, and `prisma migrate status` could not complete database inspection. Atomic Accounting Success Audit Integration is code-complete but remains partial pending live DB verification. No Prisma migration, failed/denied/auth/CRUD audit integration, accounting periods, inventory/COGS, approvals, AI orchestration, reports, PDFs, dashboards, chat persistence, or mobile audit UI were added.
